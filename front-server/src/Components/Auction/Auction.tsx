...
function Auction(): JSX.Element {
  ...
  useEffect(() => {
    ...
    if (!myAuction) return;
    const sortedId = Array.isArray(getAuction?.data)
      ? getAuction?.data.slice().sort((a: any, b: any) => parseInt(a.auctionId) - parseInt(b.auctionId))
      : [];
    const sortedPriceHigh = Array.isArray(getAuction?.data)
      ? getAuction?.data.slice().sort((a: any, b: any) => b.price - a.price)
      : [];
    const sortedPriceLow = Array.isArray(getAuction?.data)
      ? getAuction?.data.slice().sort((a: any, b: any) => a.price - b.price)
      : [];
    const sortedData =
      selectSort === '최신순' ? sortedId : selectSort === '높은가격순' ? sortedPriceHigh : sortedPriceLow;

    const sortedId2 = Array.isArray(myAuction?.data)
      ? myAuction!.data.slice().sort((a: any, b: any) => parseInt(a.auctionId) - parseInt(b.auctionId))
      : [];
    const sortedPriceHigh2 = Array.isArray(myAuction?.data)
      ? myAuction!.data.slice().sort((a: any, b: any) => b.price - a.price)
      : [];
    const sortedPriceLow2 = Array.isArray(myAuction?.data)
      ? myAuction!.data.slice().sort((a: any, b: any) => a.price - b.price)
      : [];
    const sortedData2 =
      selectSort === '최신순' ? sortedId2 : selectSort === '높은가격순' ? sortedPriceHigh2 : sortedPriceLow2;
  ...
  useEffect(() => {
    ...
    setSelectItem(
      selectLevel === 'MY'
        ? myAuction!.data?.length > 0
          ? myAuction!.data[selectNum]
          : undefined
        : getAuction!.data!.length > 0
        ? getAuction?.data[selectNum]
        : undefined
    );
      ...
      <motion.div
        ...
        <div className="bg-[#ffffff] lg:p-2 px-2 py-1 lg:my-2 my-1 rounded-lg bg-opacity-40 shadow-md h-[85%]">
          <div className="flex justify-between my-1">
            ...
            <div className="flex">
              <div
                className={`hover:scale-105 active:scale-105 duration-300 transition-all ${sortStyle} ${
                  selectSort === '최신순' ? 'font-bold text-[#FF8C8C]' : ''
                }`}
                onClick={() => {
                  setSelectSort('최신순');
                  setSelectNum(0);
              ...
              <div
                className={`hover:scale-105 active:scale-105 duration-300 transition-all ${sortStyle} ${
                  selectSort === '높은가격순' ? 'font-bold text-[#FF8C8C]' : ''
                }`}
                onClick={() => {
                  setSelectSort('높은가격순');
                  setSelectNum(0);
              ...
              <div
                className={`hover:scale-105 active:scale-105 duration-300 transition-all ${sortStyle} ${
                  selectSort === '낮은가격순' ? 'font-bold text-[#FF8C8C]' : ''
                }`}
                onClick={() => {
                  setSelectSort('낮은가격순');
                  setSelectNum(0);
              ...
              <div
                className={`hover:scale-105 active:scale-105 duration-300 transition-all ${levelStyle} ${
                  selectLevel === 'MY' ? 'bg-[#FF8C8C] text-[#ffffff]' : 'text-[#a5a5a5] bg-[#ffffff]'
                } `}
                onClick={() => {
                  setSelectLevel('MY');
                  setSelectNum(0);
          ...
          <div className="flex justify-between lg:h-[90%] h-[85%] lg:my-2 my-1">
            <div className="lg:w-[63%] w-[55%] flex justify-start flex-wrap overflow-auto">
              {selectLevel !== 'MY' &&
                ...
                getAuction.data &&
                getAuction?.data?.length > 0 &&
                getAuction?.data?.map((item: any, index: number) => (
                  <div
                    key={index}
                    ...
                    <div className="text-[#7c7c7c] lg:text-[1.0rem] text-[0.8rem] border-t-[0.01rem] mx-2 truncate">
                      [
                      {item?.assetResDto.assetCategory === 'FURNITURE'
                        ? '가구'
                        : item?.assetResDto.assetCategory === 'PROP'
                        ? '소품'
                        : '집'}
                      ] {item?.assetResDto.assetNameKor}
                    </div>
              ...
              {((myAuction && myAuction?.data.length <= 0 && selectLevel === 'MY') ||
                (getAuction && getAuction?.data.length <= 0 && selectLevel !== 'MY')) && (
                <div className="text-[1.1rem] text-[#7a7a7a] mx-auto my-auto">등록된 아이템이 없습니다.</div>
              )}
            </div>
            ...
            <div className="lg:w-[35%] w-[47%] bg-[#ffffff] rounded-lg shadow flex flex-col justify-between">
              {selectItem === undefined ? (
                <>
                  <div
                    className={`lg:px-3 lg:text-[1rem] px-2 text-[0.9rem] rounded-full rounded-bl-none w-fit text-[#ffffff] font-bold bg-[#7c7c7c] `}>
                    ???
                  </div>
                  <img
                    className="block w-full lg:h-[60%] h-[54%] object-contain"
                  ...
                  <div className="mb-2">
                    ...
                    <div className="bg-[#c0c0c0] text-[#ffffff] font-bold mx-2 rounded-md py-[0.1rem] lg:text-[1.3rem] text-[0.8em] text-center px-3">
                      구매하기
                    </div>
                  </div>
                </>
                ...
                <>
                  <div
                    className={`lg:px-3 lg:text-[1rem] px-2 text-[0.9rem] rounded-full rounded-bl-none w-fit text-[#ffffff] font-bold ${
                  ...
                  <div className="mb-2">
                    <div className="lg:block flex px-2 border-t-[0.01rem] mx-2 lg:text-[1.2rem] text-[0.8rem]">
                      <div>
                        [
                        {selectItem?.assetResDto.assetCategory === 'FURNITURE'
                          ? '가구'
                          : selectItem?.assetResDto.assetCategory === 'PROP'
                          ? '소품'
                          : '집'}
                        ]{' '}
                      </div>
                    ...
                    <div className="px-2 mx-2 lg:text-[1.5rem] text-[1rem] font-bold">
                      {selectItem?.price.toLocaleString()}원
                    </div>
                    {auctionIds && getAuction && auctionIds?.includes(selectItem?.auctionId) ? (
                      <div
                        className="bg-[#000000] cursor-pointer text-[#ffffff] font-bold mx-2 rounded-md py-[0.1rem] lg:text-[1.3rem] text-[0.8em] text-center px-3"
                        onClick={() => {
                          setWhat('판매취소');
                          setIsOpen(true);
                        }}>
                      ...
                      <div
                        className="bg-[#767C77] cursor-pointer text-[#ffffff] font-bold mx-2 rounded-md hover:bg-[#000000] py-[0.1rem] lg:text-[1.3rem] text-[0.8em] text-center px-3"
                        onClick={() => {
                          setWhat('구매');
                          setIsOpen(true);
                        }}>
